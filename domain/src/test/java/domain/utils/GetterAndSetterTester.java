...
public class GetterAndSetterTester {
	...
	/**
	 * Adds a map of instances to use by default by the tester when an instance of a
	 * given class is needed. Every time an instance of a class is needed to be used
	 * on a setter or to construct a class, the class will be looked in the map and
	 * the value object will be used as instance.
	 *
	...
	/**
	 * Test the accessors of the given instance. Only instantiation through default
	 * constructors or primitive types is supported, therefore the tested class
	 * accessible fields are expected to be recursively traced to classes that can
	 * be instantiated that way, otherwise the test will throw stack overflows while
	 * trying to instantiate classes. If complicated instantiations are involved,
	 * it's recommended to use the addDefaultInstances method.
	 *
	 ...
	 * instantiated. Only instantiation through default constructors or primitive
	 * types is supported, therefore the tested class constructor arguments and
	 * accessible fields are expected to be recursively traced to classes that can
	 * be instantiated that way, otherwise the test will throw stack overflows while
	 * trying to instantiate classes. If complicated instantiations are involved,
	 * it's recommended to use the addDefaultInstances method.
	...
	/**
	 * Returns the getter of the field or null if there is none. Both get(FieldName)
	 * and is(FieldName) method will be checked. If the method return type is
	 * different from field type, the method will be ignored.
	 *
	...
	/**
	 * Returns the name of the field with its first character as a capital letter.
	 * useful to append it to a method name such as a getter or setter.
	 *
	...
	/**
	 * Test the getter and setter method of the field, using the given instance of
	 * the class.
	 *
	...
	private void testGetterAndSetter(final Field field, final Object instance) {
		...
		try {
			...
			assertEquals("Failed getter and setter test of field " + field.getName() + " on classs "
	...
	private void testSetter(final Field field, final Object instance) {
		...
		try {
			...
			assertEquals("Failed setter test of field " + field.getName() + " on classs "
	...
	private void testGetter(final Field field, final Object instance) {
		...
		try {
			...
			assertEquals("Failed getter test of field " + field.getName() + " on classs "
					+ field.getDeclaringClass().getName(), value, getter.invoke(instance));
	 ...
	 * instances will be checked for an instance. If not existent, then if a default
	 * (no arguments) constructor exists, it will be used, otherwise any other
	 * constructor will be tried until the class can be instantiated. The last
	 * choice will be returning null.
	 *
	...
	/**
	 * Sets a list of the names of the fields that should be ignored. All the fields
	 * in the list won't be tested.
	 *
